...
describe('via api', () => {
  it('works as an ESM import', async () => {
    ...
    const initialFileContents = `
      ...
      import remarkIgnore, { ignoreStart, ignoreEnd } from 'mdast-util-tight-comments';
      import defaultIgnoreStart from 'mdast-util-tight-comments/start';
      import defaultIgnoreEnd from 'mdast-util-tight-comments/end';
      ...
      },
      {
        initialFileContents: {
          ...
          'package.json': JSON.stringify({
            name: 'dummy-pkg',
            remarkConfig: {
              plugins: ['mdast-util-tight-comments', 'remark-reference-links']
            }
          })
        },
        runWith: {
    ...
    await withMockedFixture(
      ...
      },
      {
        initialFileContents: {
          'README.md': getFixtureString('ignore-range-2'),
          '.remarkrc.mjs': `
            import { ignoreStart } from 'mdast-util-tight-comments';
            export default {
              plugins: [
                ignoreStart,
                'reference-links',
